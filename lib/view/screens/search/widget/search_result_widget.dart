...
class SearchResultWidgetState extends State<SearchResultWidget> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GetBuilder<SearchControllerNew>(builder: (searchController) {
        bool isNull = true;
        int length = 0;
      ...
      Expanded(child: NotificationListener(
        onNotification: (dynamic scrollNotification) {
          if (scrollNotification is ScrollEndNotification) {
            Get.find<SearchControllerNew>().setStore(_tabController!.index == 1);
            Get.find<SearchControllerNew>().searchData(widget.searchText, false);
          }
          return false;
        },
        child: TabBarView(
