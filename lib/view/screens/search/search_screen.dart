...
class SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      Get.find<SearchControllerNew>().getSuggestedItems();
    }
    Get.find<SearchControllerNew>().getHistoryList();
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if(Get.find<SearchControllerNew>().isSearchMode) {
          return true;
        }else {
          Get.find<SearchControllerNew>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
        ...
        body: SafeArea(child: Padding(
          ...
          child: GetBuilder<SearchControllerNew>(builder: (searchController) {
  ...
  void _actionSearch(bool isSubmit, String? queryText, bool fromHome) {
    if(Get.find<SearchControllerNew>().isSearchMode || isSubmit) {
      if(queryText!.isNotEmpty) {
        Get.find<SearchControllerNew>().searchData(queryText, fromHome);
      }else {
      ...
      List<double?> prices = [];
      if(!Get.find<SearchControllerNew>().isStore) {
        for (var product in Get.find<SearchControllerNew>().allItemList!) {
          prices.add(product.price);
        }
        prices.sort();
      }
      double? maxValue = prices.isNotEmpty ? prices[prices.length-1] : 1000;
      Get.dialog(FilterWidget(maxValue: maxValue, isStore: Get.find<SearchControllerNew>().isStore));
    }
  }
}
